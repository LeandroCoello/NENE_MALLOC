...
namespace FrbaCommerce.Login
{
    class LoginDatos
    {
        public static int logeoSql(string nombreUsuario, string passHaseada) {
            ...
            string querito = "exec SQL_O.proc_login User_Id,Userpass FROM SQL_O.Usuario where (User_Id ='" +nombreUsuario+"')and (Userpass = '" +passHaseada+"')";
        ...
        SQLConnector conec = SQLConnector.getInstance();
        string querito = "SELECT User_Id,Userpass FROM SQL_O.Usuario where (User_Id ='" + nombreUsuario + "')";
